...
@section('title', 'تصنيفات المقالات والأعمدة')

@section('content')
    <div x-data="postCategoryModal()" class="max-w-7xl mx-auto pb-20">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
            <div>
                <h1 class="text-3xl font-black text-gray-900 tracking-tight flex items-center gap-3">
                    ...
                    تصنيفات المقالات والأعمدة
                </h1>
                <p class="text-gray-500 font-bold mt-2 text-sm">إدارة الأقسام لفرز وتصنيف المقالات والأعمدة.</p>
            </div>
        ...
        <div class="bg-white rounded-[2rem] shadow-sm border border-gray-50 overflow-hidden relative">
                        ...
                        <tr class="hover:bg-gray-50/50 transition-colors group">
                            ...
                            <td class="px-6 py-5 align-middle">
                                <div class="flex items-center justify-center gap-2 opacity-80 group-hover:opacity-100 transition-opacity">
                                    <button @click="openEdit({{ $category->id }}, '{{ addslashes($category->name['ar'] ?? '') }}', '{{ addslashes($category->name['en'] ?? '') }}')"
                                    ...
                                    <form action="{{ route('admin.post-categories.destroy', $category->id) }}" method="POST" class="delete-form m-0">
                                        @csrf
                                        @method('DELETE')
            ...
            @if($categories->hasPages())
                <div class="p-6 border-t border-gray-50 bg-gray-50/30">{{ $categories->links() }}</div>
            @endif
        </div>
                            ...
                            <form :action="formAction" method="POST" class="p-8" @submit="isSubmitting = true">
                                ...
                                <div class="space-y-6">
                                    <div class="space-y-2 relative">
                                        ...
                                        <input type="text" name="name[ar]" x-model="formData.ar" required placeholder="مثال: آراء وأعمدة" class="w-full pl-14 pr-4 py-3.5 bg-gray-50 border border-gray-200 rounded-2xl focus:bg-white focus:border-primary/50 focus:ring-4 focus:ring-primary/10 transition-all font-bold">
                                    </div>
                                    ...
                                    <div class="space-y-2 relative" dir="ltr">
                                        ...
                                        <input type="text" name="name[en]" x-model="formData.en" required placeholder="e.g: Columns" class="w-full pr-14 pl-4 py-3.5 bg-gray-50 border border-gray-200 rounded-2xl focus:bg-white focus:border-primary/50 focus:ring-4 focus:ring-primary/10 transition-all font-bold text-left">
                                    </div>
                                </div>
...
@section('scripts')
    <script>
        function postCategoryModal() {
            return {
                isOpen: false,
                isEdit: false,
                isSubmitting: false,
                formAction: '{{ route("admin.post-categories.store") }}',
                baseUrl: '{{ url("admin/post-categories") }}',
                formData: { ar: '', en: '' },

                openAdd() {
                    this.isEdit = false;
                    this.isSubmitting = false;
                    this.formAction = '{{ route("admin.post-categories.store") }}';
                    this.formData = { ar: '', en: '' };
                    this.isOpen = true;
                },
