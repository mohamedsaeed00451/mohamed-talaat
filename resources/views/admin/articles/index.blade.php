...
@section('content')
    <div class="max-w-7xl mx-auto pb-20">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
            ...
            <a href="{{ route('admin.articles.create') }}" class="cursor-pointer group flex items-center gap-2 px-6 py-3.5 bg-primary text-white rounded-2xl font-black text-sm shadow-xl shadow-primary/30 hover:-translate-y-1 transition-all duration-300">
                    ...
                    @forelse($articles as $article)
                        <tr class="hover:bg-gray-50/50 transition-colors group">
                            ...
                            <td class="px-6 py-5 align-middle">
                                @if($article->image)
                                    <img src="{{ asset($article->image) }}" class="w-12 h-12 rounded-xl object-cover shadow-sm border border-gray-100">
                                @else
                            ...
                            <td class="px-6 py-5 align-middle">
                                <p class="font-black text-gray-800 text-sm">{{ $article->title['ar'] ?? '' }}</p>
                                <p class="font-bold text-gray-400 text-xs mt-1" dir="ltr">{{ $article->title['en'] ?? '' }}</p>
                            </td>

                            <td class="px-6 py-5 text-center align-middle">
                                <span class="px-3 py-1 bg-indigo-50 text-indigo-700 border border-indigo-100 rounded-lg text-xs font-black">{{ $article->type->name['ar'] ?? 'غير محدد' }}</span>
                            </td>
                            ...
                            <td class="px-6 py-5 align-middle sticky left-0 bg-white group-hover:bg-gray-50 z-10 shadow-[-10px_0_15px_-3px_rgba(0,0,0,0.03)] border-r border-gray-50 transition-colors">
                                <div class="flex items-center justify-center gap-2">
                                    <a href="{{ route('admin.articles.edit', $article->id) }}"
                                       title="تعديل المقال"
                                       class="p-2.5 bg-blue-50 text-blue-500 hover:bg-blue-500 hover:text-white rounded-xl transition-all duration-300 hover:shadow-lg hover:shadow-blue-500/30 hover:-translate-y-1">
