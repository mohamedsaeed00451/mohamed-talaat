                    ...
                    @forelse($articles as $article)
                        <tr class="hover:bg-gray-50/50 transition-colors group">
                            ...
                            <td class="px-6 py-5 align-middle">
                                ...
                                <p class="font-bold text-gray-400 text-[10px] mt-1 flex items-center gap-1">
                                    @if($article->published_at && \Carbon\Carbon::parse($article->published_at)->isFuture())
                                        <span
                                            class="text-orange-500">مجدول: {{ \Carbon\Carbon::parse($article->published_at)->format('Y-m-d H:i') }}</span>
                                    @else
                            ...
                            <td class="px-6 py-5 text-center align-middle">
                                <span
                                    class="px-3 py-1 bg-indigo-50 text-indigo-700 border border-indigo-100 rounded-lg text-[11px] font-black">{{ $article->type->name['ar'] ?? 'غير محدد' }}</span>
                            </td>

                            <td class="px-6 py-5 align-middle">
                                <div class="flex flex-col gap-2"
                                     x-data="articleStatus({{ $article->id }}, {{ $article->is_active ? 'true' : 'false' }}, {{ $article->is_featured ? 'true' : 'false' }}, {{ $article->is_old ? 'true' : 'false' }})">
